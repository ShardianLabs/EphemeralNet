...
- **Advertised endpoints**: Transport auto-discovery now runs even when the control plane stays on loopback. As soon as the daemon starts listening it samples STUN and publishes the resulting transport host/port (or falls back to the first candidate). Use `advertise_control_host`/`port` (or explicit `advertised_endpoints`) whenever you need a branded hostname, a relay address, or deterministic ordering. Run `eph defaults` after changes to confirm the advertised list (manual entries first, auto discoveries next) matches what you intend to publish.
