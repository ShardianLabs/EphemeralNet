...
- **Port forwarding**: If peers must reach your control plane directly, forward TCP 47777 (and any custom control port) from the router or cloud firewall to the daemon host. Data-plane handshakes bind to TCP 45000 by default; forward/allow that port as well when you expect direct swarm ingress. Verify externally via `nc -vz <ip> 47777` and `nc -vz <ip> 45000` (or `Test-NetConnection -ComputerName <ip> -Port <n>`). When forwarding fails due to carrier-grade NAT, relocate the daemon to a VPS or push traffic through a TURN/STUN relay as described in `ops/bootstrap/README.md`.
- **Advertised endpoints**: Transport auto-discovery now runs even when the control plane stays on loopback. As soon as the daemon starts listening it samples UPnP/STUN and publishes the resulting transport host/port (or falls back to the first candidate). Use `advertise_control_host`/`port` (or explicit `advertised_endpoints`) whenever you need a branded hostname, a relay address, or deterministic ordering. Run `eph defaults` after changes to confirm the advertised list (manual entries first, auto discoveries next) matches what you intend to publish.
- **Shardian defaults**: Out of the box every node contacts `bootstrap1.shardian.com` and `bootstrap2.shardian.com` on TCP 45000 and probes the shared relays `stun.shardian.com` and `turn.shardian.com`. Keep outbound firewall rules open to these hosts if you rely on the public mesh, or override `bootstrap_nodes`/`NatTraversal` settings in your config when you operate a private federation.
