...
- `discovery_hints`: Ordered list of `(scheme, transport, endpoint, priority)` tuples that the CLI can consult when it must bootstrap a manifest-only fetch. `scheme` communicates the high-level protocol (`control`, `transport`, `https`, etc.), while `transport` differentiates the concrete wiring (`control` for the line-oriented daemon channel, `tcp` for the swarm data plane, reserved for future QUIC/HTTP variants). Manually advertised endpoints (`--advertise-control`/`advertised_endpoints.manual`) are tagged as `scheme="control"`/`transport="control"`, whereas auto-discovered data-plane endpoints surface as `scheme="transport"`/`transport="tcp"`. Lower `priority` numbers are attempted first.
