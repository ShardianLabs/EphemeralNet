...
int main() {
    ...
    const bool initiated = node_a.perform_handshake(node_b.id(), node_b.public_identity());
    assert(initiated);
    const bool accepted = node_b.perform_handshake(node_a.id(), node_a.public_identity());
    assert(accepted);
    ...
    const bool repeated = node_a.perform_handshake(node_b.id(), node_b.public_identity());
    assert(repeated);
    ...
    const bool failed = node_a.perform_handshake(invalid_peer, 0u);
    assert(!failed);
